    ...
    calculateHeaderCode(columnNumber) {
        ...
        while (columnNumber > 0) {
            ...
            if (rem === 0) {
                ...
                columnName.push(
                    String.fromCharCode(rem - 1 + "A".charCodeAt(0))
                );
    ...
    handleMouseDown(event) {
        ...
        for (let i = 1; i < this.verticalLines.length; i++) {
            if (this.verticalLines[i].isCloseToBorder(x, y)) {
                ...
                this.startX = x;
                this.initialWidth =
                    this.columnsWidth[this.resizeColumnIndex - 1];
                break;
            }
        }
        if (!flag) {
            for (
                let i = this.currentRow + 1;
                i < this.horizontalLines.length;
                i++
            ) {
                if (this.horizontalLines[i].isCloseToBorder(x, y)) {
                    ...
                    this.startY = y;
                    this.initialHeight =
                        this.rowsHeight[this.resizeRowIndex - 1];
                    break;
                }
            }
        }
                ...
                if (this.data[1][cell.gridColumn].text == "email") {
                    for (let i = 1; i < csv_columns; i++) {
                        if (cell.gridColumn == i) {
                            ...
                            formObject[this.data[1][i].text] =
                                this.data[cell.gridRow][i].text;
                        }
                    }
        ...
        if (this.isSelecting == false && this.selectedCells.length <= 3) {
            ...
            if (keyPressed == "ArrowRight") {
                ...
                if (current_cell.gridColumn + 1 < this.columns) {
                    ...
                    next_cell =
                        this.data[current_cell.gridRow][
                            current_cell.gridColumn + 1
                        ];
            ...
            if (keyPressed == "ArrowUp") {
                if (current_cell.gridRow - 1 >= 0) {
                    next_cell =
                        this.data[current_cell.gridRow - 1][
                            current_cell.gridColumn
                        ];
    ...
    updateMetaData(cell) {
        ...
        if (text) {
            ...
            this.metaData.average = (
                this.metaData.sum / this.metaData.count
            ).toFixed(2);
        }
    }
    clearMetaData() {
    ...
    updateSelect() {
        ...
        for (let i = this.currentRow; i < this.rows; i++) {
            for (let j = 0; j < this.columns; j++) {
                ...
                const cellRight =
                    cell.xPosition + this.columnsWidth[cell.gridColumn];
                const cellBottom =
                    cell.yPosition + this.rowsHeight[cell.gridRow];
